...
class test1_:
    ...
    def sanitize(self, lineTemp, lineNo):
        lineTemp = re.sub("TestValue", "TestValueUnit,P/F", lineTemp)
                    ...
                    if(counter > 1):
                        ...
                        if(re.match("TstNum,Pin,Chn",lineTemp) or (((len(lineFinal) is 10) or (len(lineFinal) is 11))) and re.match("[0-9]+?,[A-Z0-9]+?,[a-z0-9_]+?,[A-Z0-9_]+?,[a-zA-Z_]+?",lineTemp)):
                            ...
                            if(not (lineFinal[0]=="TstNum")):
                                ...
                                lineFinal.insert(8, re.match("[0-9.]+(?=_)", lineFinal[8]).group(0))
                                lineFinal[9] = re.sub("[0-9._]+","",lineFinal[9])
                            else:
                                lineFinal.insert(8,'TestValue(exclude unit)')
...
instance = test1_()
instance.makeAllCSVs("101_TT_25C.txt","tempfile")
